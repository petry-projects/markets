...
@test "payload includes the OrganizationAdmin bypass actor with bypass_mode always" {
  ...
  jq -e '.bypass_actors == [{"actor_type": "OrganizationAdmin", "bypass_mode": "always"}]' "$BATS_TEST_TMPDIR/payload.json"
}
...
@test "update preserves existing bypass actors while ensuring OrganizationAdmin is present" {
  ...
  jq -e '.bypass_actors == [{"actor_type":"RepositoryRole","bypass_mode":"pull_request"},{"actor_type":"OrganizationAdmin","bypass_mode":"always"}]' "$BATS_TEST_TMPDIR/payload.json"
}
