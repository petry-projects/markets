...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  // Fix tslib CJS/ESM interop on web — catch both bare 'tslib' imports
  // and subpath imports like 'tslib/modules/index.js' that use CJS wrappers
  if (platform === 'web' && (moduleName === 'tslib' || moduleName.startsWith('tslib/'))) {
    return {
  ...
  // Shim native-only auth packages on web
  ...
  if (platform === 'web' && moduleName === '@react-native-google-signin/google-signin') {
    return {
      filePath: path.resolve(__dirname, 'lib/google-signin-web-shim.js'),
      type: 'sourceFile',
    };
  }
  if (platform === 'web' && moduleName === 'expo-apple-authentication') {
    return {
      filePath: path.resolve(__dirname, 'lib/apple-auth-web-shim.js'),
      type: 'sourceFile',
    };
  }
