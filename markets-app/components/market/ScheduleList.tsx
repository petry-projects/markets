    ...
    <VStack className="gap-4">
      ...
      {recurring.length > 0 && (
        <VStack className="gap-2">
          ...
          {recurring.map((s) => (
            <ScheduleCard key={s.id} schedule={s} onEdit={onEdit} onDelete={confirmDelete} isRecurring />
          ))}
        </VStack>
      )}
      ...
      {oneTime.length > 0 && (
        <VStack className="gap-2">
          ...
          {oneTime.map((s) => (
            <ScheduleCard key={s.id} schedule={s} onEdit={onEdit} onDelete={confirmDelete} isRecurring={false} />
          ))}
        </VStack>
      )}
...
function ScheduleCard({ schedule, onEdit, onDelete, isRecurring }: ScheduleCardProps) {
  const bgClass = isRecurring ? 'bg-primary-50 border-primary-200' : 'bg-warning-50 border-warning-200';
