...
@test "payload includes the OrganizationAdmin bypass actor with bypass_mode always" {
  ...
  jq -e '.bypass_actors == [{"actor_id":0,"actor_type":"OrganizationAdmin","bypass_mode":"always"},{"actor_id":3167543,"actor_type":"Integration","bypass_mode":"always"}]' "$TEST_TMPDIR/payload.json"
}
...
@test "update preserves existing bypass actors while ensuring OrganizationAdmin is present" {
  ...
  jq -e '.bypass_actors == [{"actor_type":"RepositoryRole","bypass_mode":"pull_request"},{"actor_id":0,"actor_type":"OrganizationAdmin","bypass_mode":"always"},{"actor_id":3167543,"actor_type":"Integration","bypass_mode":"always"}]' "$TEST_TMPDIR/payload.json"
}
